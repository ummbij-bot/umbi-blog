...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    keywords: ['tech', 'technology', ...post.title.split(' ').slice(0, 5)],
...
export default function BlogPost({ params }: PageProps) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'Tech',
        item: 'https://umbi-blog.vercel.app/tech',
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: post.title,
        item: `https://umbi-blog.vercel.app/tech/${post.slug}`,
      },
    ],
  };
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://umbi-blog.vercel.app/tech/${post.slug}`,
    },
  };
      ...
      <article className="min-h-screen bg-neutral-50">
        ...
        <div className="container-custom py-12">
          <div className="max-w-3xl mx-auto">
            {/* Category Badge */}
            <div className="mb-6">
              <span className="inline-block px-4 py-2 rounded-full text-sm font-bold bg-blue-100 text-blue-700">
                Tech
              </span>
            </div>
            ...
            {/* Excerpt */}
            <div className="text-xl text-neutral-700 leading-relaxed mb-8 bg-neutral-100 p-6 rounded-2xl border-l-4 border-blue-500">
              {post.excerpt}
            </div>
            ...
            <div className="prose prose-lg max-w-none">
              <ReactMarkdown
                components={{
                  ...
                  blockquote: (props) => (
                    <blockquote className="border-l-4 border-blue-500 pl-4 italic my-4 text-neutral-700 bg-neutral-50 py-2" {...props} />
                  ),
                  code: ({ inline, children, ...props }: { inline?: boolean; children?: React.ReactNode }) =>
                    inline ? (
                      <code className="bg-neutral-100 px-2 py-1 rounded text-sm font-mono text-blue-700" {...props}>
                        {children}
                      </code>
                    ) : (
            ...
            <div className="mt-12 pt-8 border-t border-neutral-200">
              <div className="flex items-center gap-4">
                <span className="font-bold text-neutral-900">Share:</span>
                  ...
                  <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`https://umbi-blog.vercel.app/tech/${post.slug}`)}`}
                  target="_blank"
                ...
                >
                  ...
                  <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://umbi-blog.vercel.app/tech/${post.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
